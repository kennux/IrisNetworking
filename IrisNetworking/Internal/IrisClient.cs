            ...
            if (this.isServerClient)
            {
                ...
                switch (header)
                {
                    ...
                    case 0:
                        ...
                        {
                            // TODO: Maybe cache last view state?
                            IrisStream s = new IrisStream(this.master);
                            v.Serialize(s);
                            this.SendMessage(new IrisInstantiationMessage(this.master.GetLocalPlayer(), v.GetObjectName(), v.GetViewId(), v.GetOwner(), s.GetBytes()));
